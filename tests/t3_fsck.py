...
class TestFsck:
    ...
    def setup(self, tmp_path):
        ...
        self.raw_backend = run_async(
            local.AsyncBackend.create, Namespace(storage_url='local://' + self.backend_dir)
        )
        self.backend = run_async(AsyncComprencBackend.create, None, (None, 0), self.raw_backend)
        self.cachedir = str(tmp_path / 'cache')
        os.makedirs(self.cachedir)
        self.max_obj_size = 1024

        self.db = Connection(str(tmp_path / 'test.db'))
    ...
    async def assert_fsck(self, *fns, can_fix=True):
    ...
    async def test_cache(self):
        ...
        await self.assert_fsck(self.fsck.check_cache)
        assert (await self.backend.fetch('s3ql_data_1'))[0] == b'somedata'
    ...
    async def test_lof1(self):
        ...
        await self.assert_fsck(check)

    @pytest.mark.trio
    async def test_lof2(self):
        # Remove lost+found
        ...
        await self.assert_fsck(self.fsck.check_lof)

    @pytest.mark.trio
    async def test_lof_issue_273(self):
        name_id = self.db.get_val('SELECT id FROM names WHERE name=?', (b'lost+found',))
        inode = self.db.get_val(
            'SELECT inode FROM contents WHERE name_id=? AND parent_inode=?',
            (name_id, ROOT_INODE),
    ...
    @pytest.mark.trio
    async def test_wrong_inode_refcount(self):
        inode = self.db.rowid(
            "INSERT INTO inodes (mode,uid,gid,mtime_ns,atime_ns,ctime_ns,refcount,size) "
            "VALUES (?,?,?,?,?,?,?,?)",
            (
        ...
        await self.assert_fsck(self.fsck.check_inodes_refcount)

    @pytest.mark.trio
    async def test_orphaned_inode(self):
        self.db.rowid(
            "INSERT INTO inodes (mode,uid,gid,mtime_ns,atime_ns,ctime_ns,refcount,size) "
            "VALUES (?,?,?,?,?,?,?,?)",
            (
    ...
    @pytest.mark.trio
    async def test_name_refcount(self):
        inode = self.db.rowid(
            "INSERT INTO inodes (mode,uid,gid,mtime_ns,atime_ns,ctime_ns,refcount,size) "
            "VALUES (?,?,?,?,?,?,?,?)",
            (
        ...
        await self.assert_fsck(self.fsck.check_names_refcount)

    @pytest.mark.trio
    async def test_orphaned_name(self):
        self._add_name(b'zupbrazl')
        await self.assert_fsck(self.fsck.check_names_refcount)

    @pytest.mark.trio
    async def test_contents_inode(self):
        self.db.execute(
            'INSERT INTO contents (name_id, inode, parent_inode) VALUES(?,?,?)',
            (self._add_name(b'foobar'), 124, ROOT_INODE),
        )

        await self.assert_fsck(self.fsck.check_contents_inode)

    @pytest.mark.trio
    async def test_contents_inode_p(self):
        inode = self.db.rowid(
            "INSERT INTO inodes (mode,uid,gid,mtime_ns,atime_ns,ctime_ns,refcount,size) "
            "VALUES (?,?,?,?,?,?,?,?)",
            (
        ...
        await self.assert_fsck(self.fsck.check_contents_parent_inode)

    @pytest.mark.trio
    async def test_contents_name(self):
        inode = self.db.rowid(
            "INSERT INTO inodes (mode,uid,gid,mtime_ns,atime_ns,ctime_ns,refcount,size) "
            "VALUES (?,?,?,?,?,?,?,?)",
            (
        ...
        await self.assert_fsck(self.fsck.check_contents_name)
    ...
    @pytest.mark.trio
    async def test_inodes_size(self):
        id_ = self.db.rowid(
            "INSERT INTO inodes (mode,uid,gid,mtime_ns,atime_ns,ctime_ns,refcount,size) "
            "VALUES (?,?,?,?,?,?,?,?)",
            (
        ...
        await self.backend.store('s3ql_data_%d' % obj_id, b'foo')
        ...
        await self.assert_fsck(self.fsck.check_inodes_size)
    ...
    @pytest.mark.trio
    async def test_objects_id(self):
        # Create an object that only exists in the backend
        await self.backend.store('s3ql_data_4364', b'Testdata')
        await self.assert_fsck(self.fsck.check_objects_id)
    ...
    @pytest.mark.trio
    async def test_objects_hash(self):
        id_ = self.db.rowid(
            "INSERT INTO inodes (mode,uid,gid,mtime_ns,atime_ns,ctime_ns,refcount,size) "
            "VALUES (?,?,?,?,?,?,?,?)",
            (
    ...
    async def test_missing_obj(self):
        ...
        self._link(b'test-entry', id_)
        await self.assert_fsck(self.fsck.check_objects_id, self.fsck.check_inode_blocks_obj_id)

    @pytest.mark.trio
    async def test_inode_blocks_inode(self):
        obj_id = self.db.rowid(
        ...
        await self.assert_fsck(self.fsck.check_inode_blocks_inode)

    @pytest.mark.trio
    async def test_inode_blocks_obj_id(self):
        id_ = self.db.rowid(
            "INSERT INTO inodes (mode,uid,gid,mtime_ns,atime_ns,ctime_ns,refcount,size) "
            "VALUES (?,?,?,?,?,?,?,?)",
            (
        ...
        self._link(b'test-entry', id_)
        await self.assert_fsck(self.fsck.check_inode_blocks_obj_id)

    @pytest.mark.trio
    async def test_symlinks_inode(self):
        self.db.execute(
            'INSERT INTO symlink_targets (inode, target) VALUES(?,?)', (42, b'somewhere else')
        )

        await self.assert_fsck(self.fsck.check_symlinks_inode)

    @pytest.mark.trio
    async def test_ext_attrs_inode(self):
        self.db.execute(
            'INSERT INTO ext_attributes (name_id, inode, value) VALUES(?,?,?)',
            (self._add_name(b'some name'), 34, b'some value'),
        )

        await self.assert_fsck(self.fsck.check_ext_attributes_inode)

    @pytest.mark.trio
    async def test_ext_attrs_name(self):
        id_ = self.db.rowid(
            "INSERT INTO inodes (mode,uid,gid,mtime_ns,atime_ns,ctime_ns,refcount,size) "
            "VALUES (?,?,?,?,?,?,?,?)",
            (
    ...
    async def test_loops(self):
        ...
        await self.assert_fsck(self.fsck.check_loops)

    @pytest.mark.trio
    async def test_tmpfile(self):
        # Ensure that path exists
        objname = 's3ql_data_38375'
        await self.backend.store(objname, b'bla')
        await self.backend.delete(objname)
        path = self.raw_backend._key_to_path(objname)
        tmpname = '%s#%d-%d.tmp' % (path, os.getpid(), _thread.get_ident())
        with open(tmpname, 'wb') as fh:
            fh.write(b'Hello, world')

        await self.assert_fsck(self.fsck.check_objects_temp)

    @pytest.mark.trio
    async def test_obj_refcounts(self):
        obj_id = self.db.rowid(
        ...
        await self.assert_fsck(self.fsck.check_objects_refcount)

    @pytest.mark.trio
    async def test_orphaned_obj(self):
        self.db.rowid(
    ...
    async def test_unix_size(self):
        ...
        await self.assert_fsck(self.fsck.check_unix, can_fix=False)

    @pytest.mark.trio
    async def test_unix_size_symlink(self):
        inode = 42
    ...
    @pytest.mark.trio
    async def test_unix_target(self):
        inode = 42
        self.db.execute(
    ...
    @pytest.mark.trio
    async def test_unix_nomode_reg(self):
        ...
        await self.assert_fsck(self.fsck.check_unix)
    ...
    @pytest.mark.trio
    async def test_unix_nomode_dir(self):
        perms = stat.S_IRUSR | stat.S_IWUSR | stat.S_IROTH | stat.S_IRGRP
        stamp = time_ns()
        inode = self.db.rowid(
            "INSERT INTO inodes (mode,uid,gid,mtime_ns,atime_ns,ctime_ns,refcount) "
    ...
    @pytest.mark.trio
    async def test_unix_symlink_no_target(self):
        inode = self.db.rowid(
            "INSERT INTO inodes (mode,uid,gid,mtime_ns,atime_ns,ctime_ns,refcount) "
            "VALUES (?,?,?,?,?,?,?)",
            (
    ...
    @pytest.mark.trio
    async def test_unix_child(self):
        inode = self.db.rowid(
            "INSERT INTO inodes (mode,uid,gid,mtime_ns,atime_ns,ctime_ns,refcount) "
            "VALUES (?,?,?,?,?,?,?)",
            (
    ...
    @pytest.mark.trio
    async def test_unix_blocks(self):
